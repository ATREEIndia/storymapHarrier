...
// How much extra scroll-room to give the sticky wrapper below the pinned
// section. This is how much native page scroll it takes to "release" the
// section back into flow once internal panel scrolling is exhausted.
const RELEASE_BUFFER_VH = 100;

// Max px of panel scroll a single wheel/touch tick is allowed to apply.
// Without this, a fast fling can deliver a huge deltaY in one event, jump
// the panel almost straight to its scroll boundary, and let the very next
// (still-fast) tick leak through as native scroll — blowing through the
// release buffer and un-sticking the section almost instantly. Clamping
// forces a fast gesture into a quick sequence of small steps instead of
// one big jump, so the section stays pinned through the whole journey
...
export default function ScrollMap() {
  ...
  useEffect(() => {
    ...
    // ── Delta clamp ────────────────────────────────────────────────────
    // Caps how much panel scroll a single wheel/touch event can apply, so
    // fast flings advance through locations as a quick series of small
    // steps rather than one giant jump that skips straight to the bottom.
    const clampDelta = (delta: number): number => {
      const capped = Math.min(Math.abs(delta), MAX_STEP);
    ...
    // top going up). When at a boundary, handlers must NOT preventDefault
    // — the event needs to leak through to the page so the sticky section
    // can release and native scroll can continue past it.
    const atBoundary = (delta: number): boolean => {
      const max = panel.scrollHeight - panel.clientHeight;
      if (delta > 0 && panel.scrollTop >= max - 1) return true;
      if (delta < 0 && panel.scrollTop <= 0) return true;
      return false;
    };
    ...
    // panel (as opposed to the map-redirect path above). No boundary check
  ...
  return (
    // Sticky wrapper: gives the pinned section room to hold in place while
    // scrolled through, then release. Height = 100vh (the pinned section
